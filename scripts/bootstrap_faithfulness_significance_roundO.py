...
(imported, not reimplemented) against the roundO faithfulness
regeneration -- both faithfulness_sample_baselines.csv and _novel.csv
predated the exact-match fix, BM25 retuning, and (for vector_only
specifically) the Banglish-expansion embedding retrain; re-generated
against the identical 40/50 query_ids under the current system
(see CLAUDE.md.md for the regeneration log).

Usage: python scripts/bootstrap_faithfulness_significance_roundO.py
"""
...
OUT_PATH = ROOT / "results" / "faithfulness_bootstrap_significance_roundO.csv"


def main():
    base = pd.read_csv(ROOT / "results" / "faithfulness_sample_baselines_roundO_faithfulness.csv")
    novel = pd.read_csv(ROOT / "results" / "faithfulness_sample_novel_roundO_faithfulness.csv")
