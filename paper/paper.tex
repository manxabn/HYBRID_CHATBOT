...
\begin{table}[t]
\caption{Cross-encoder reranker on (fine-tuned) vs.\ off, otherwise identical pipeline, matched vs.\ full hybrid and BM25-only ($n=199$). Both rows re-measured 2026-07-31 under the current, hard-negative-mined embedding model (the reranker-on row was previously stale, dated 2026-07-28, before this retrain -- see discussion below).}\label{tab:reranker-ablation} \centering
\begin{tabular}{|l|c|c|c|}
\hline
Reranker & Comparison & Mean diff & $p$ \\
\hline
On (fine-tuned) & vs.\ Full Hybrid, BLEU & $-0.040$ & \textbf{0.030} \\
On (fine-tuned) & vs.\ Full Hybrid, METEOR & $-0.030$ & \textbf{0.027} \\
On (fine-tuned) & vs.\ BM25-only, BLEU & $-0.044$ & \textbf{0.011} \\
On (fine-tuned) & vs.\ BM25-only, ROUGE-L & $-0.030$ & \textbf{0.016} \\
On (fine-tuned) & vs.\ BM25-only, METEOR & $-0.042$ & \textbf{0.001} \\
...
Table~\ref{tab:reranker-ablation} reports the fair, non-confounded comparison, re-measured under the current embedding model (the reranker-on condition was previously only tested pre-retrain, 2026-07-28 -- the same staleness class already found and corrected in Section~\ref{subsec:novel-vs-baselines}; re-running it here closes that gap rather than leave it unverified). Fine-tuning does not flip the reranker from negative to positive: with it enabled, the pipeline loses significantly to full hybrid on BLEU and METEOR ($p=0.030$, $p=0.027$; ROUGE-L is directionally worse but not significant, $p=0.050$) and loses significantly to BM25-only on BLEU, ROUGE-L, and METEOR ($p=0.011$, $p=0.016$, $p=0.001$; BERTScore is directionally worse and reaches significance under the paired $t$-test only, $p=0.020$, not the matched Wilcoxon test, $p=0.081$, so treated as inconclusive). The qualitative conclusion is unchanged from the pre-retrain measurement -- the reranker still loses, now confirmed rather than assumed current -- but the specific margins are larger under the current model, since BM25-only itself is now a harder baseline to beat (Section~\ref{subsec:novel-vs-baselines}). Every other result in this paper uses the reranker-off configuration: the negative result is not an artifact of an undertrained reranker, and reranker-off remains the deployed default even though, as Section~\ref{subsec:novel-vs-baselines} also reports, it does not beat BM25-only on generation-quality metrics either -- disabling the reranker avoids making an already-unfavorable comparison against BM25-only worse still.
