...
Table~\ref{tab:reranker-ablation} reports the fair, non-confounded comparison. Fine-tuning does not flip the reranker from negative to positive: with it enabled, the pipeline loses significantly to full hybrid on BLEU, BERTScore, and METEOR ($p<0.001$, $p=0.007$, $p=0.005$; ROUGE-L is directionally worse but not significant, $p=0.078$), and shows no advantage over BM25-only either (BLEU $p=0.066$, not significant). Every other result in this paper uses the reranker-off configuration: the negative result is not an artifact of an undertrained reranker, and reranker-off remains the deployed default even though, as Section~\ref{subsec:novel-vs-baselines} now reports under the current embedding model, it does not beat BM25-only on generation-quality metrics either -- disabling the reranker avoids making an already-unfavorable comparison against BM25-only worse still.
