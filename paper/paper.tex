...
\begin{table}[t]
\caption{Prerequisite-graph augmentation, isolated ablation on the 12 chain-triggering queries, before and after the chain-notation fix. The post-fix row was re-measured 2026-08-01 under the current retriever (exact-match and BM25 fixes both postdate the original post-fix measurement); see discussion below.}\label{tab:graph-ablation} \centering
\begin{tabular}{|l|c|c|c|c|}
\hline
Condition & BLEU & ROUGE-L & BERTSc. & METEOR \\
\hline
Graph on (arrow notation, pre-fix) & 0.599 & 0.928 & 0.967 & 0.869 \\
Graph on (hyphen notation, post-fix) & 0.917 & 0.961 & 0.992 & 0.960 \\
Graph off (plain retrieval) & 0.755 & 0.869 & 0.968 & 0.877 \\
\hline
\end{tabular}
\end{table}

After the fix, graph augmentation's point estimate is higher than plain retrieval on all four metrics (Table~\ref{tab:graph-ablation}), reversing the pre-fix BLEU regression entirely. This row has itself been re-measured once since the original post-fix run, for the same reason as every other ablation in this paper touching $\lambda>0$ configurations: the original measurement predated the exact-match coverage fix and the BM25 $k_1$/$b$ retuning. Under the current retriever the gap widens somewhat and the significance picture is mixed rather than uniformly negative: BLEU is significant under a paired $t$-test ($p=0.042$) but not under the matched Wilcoxon test ($p=0.063$); BERTScore shows the reverse pattern, significant under Wilcoxon ($p=0.031$) but not the paired $t$-test ($p=0.078$); ROUGE-L and METEOR remain non-significant under both ($p=0.13$, $p=0.075$ respectively, paired $t$-test). At $n=12$, this is still not a clean, both-tests-agree confirmation by the standard we apply elsewhere in this paper, so we do not claim graph augmentation is a confirmed significant win -- but the trend strengthened rather than weakened under re-measurement, and BLEU in particular moved from clearly non-significant ($p=0.10$) to borderline. We report both the bug and the fix rather than only the corrected number, since the pre-fix result would otherwise have been read as genuine evidence against graph augmentation, when it was in fact an artifact of the graph block's own text formatting not matching the corpus's convention -- a caution for any RAG component that injects templated text alongside retrieved chunks, not specific to prerequisite graphs.
...
\begin{table}[t]
\caption{Adaptive pipeline (reranker off, deployed default) vs.\ baselines, matched pairs only ($n=199/200$; 1 excluded where the pipeline abstained), re-measured 2026-08-01 under the current retriever (exact-match coverage fix and BM25 $k_1$/$b$ retuning both applied) after the previous measurement (2026-07-31, predating both fixes) was found to be stale. Positive mean diff favors the adaptive pipeline; Wilcoxon $p$ in parentheses where it disagrees with the paired $t$-test.}\label{tab:novel-sig} \centering
\begin{tabular}{|l|c|c|c|}
\hline
Comparison & Metric & Mean diff & $p$ \\
\hline
vs.\ Full Hybrid & ROUGE-L & $-0.017$ & 0.205 \\
vs.\ Full Hybrid & METEOR & $-0.013$ & 0.341 \\
vs.\ BM25-only & BERTScore & $-0.0035$ & 0.097 (0.808) \\
vs.\ BM25-only & METEOR & $-0.025$ & \textbf{0.041} (0.306) \\
vs.\ Vector-only & BLEU & $+0.012$ & 0.480 \\
vs.\ No-retrieval & BLEU & $+0.610$ & $<0.001$ \\
\hline
\end{tabular}
\end{table}

This table has itself been re-measured since the previous revision, for the same reason as the reranker rows above: the earlier version (2026-07-31) predated both the exact-match coverage fix (Section~\ref{subsec:unambiguous}) and the BM25 $k_1$/$b$ retuning (Section~\ref{subsec:novel-vs-baselines}-era discussion below), and we re-ran it under the current retriever rather than assume it was unaffected. The adaptive pipeline (reranker off) is not significantly different from full hybrid on any of the four metrics ($p\geq0.205$) or from vector-only on any of the four metrics ($p\geq0.081$ across both tests) -- a genuine flattening at the generation-quality level of what remains a clear retrieval-quality gap for vector-only specifically (Section~\ref{subsec:ir-metrics}), consistent with this paper's broader pattern of retrieval-level and generation-level results not always moving together. Against BM25-only, the picture has changed from the previous revision: point estimates remain directionally behind on all four metrics, but none now reach clean significance under both tests. METEOR comes closest, marginally significant under the paired $t$-test alone ($p=0.041$) while the Wilcoxon test does not agree ($p=0.306$); consistent with how this paper treats other test-disagreeing results elsewhere (e.g.\ Section~\ref{subsec:banglish-components}, and the previous revision's own BERTScore result in this same comparison, now itself no longer significant under either test, $p=0.097$/$0.808$), we treat this as inconclusive rather than confirmed. BLEU and ROUGE-L are directionally behind but not significant. In short, under the current retriever the adaptive pipeline is in full statistical parity with every single-method baseline it is compared against, not behind BM25-only on any metric at a confirmed significance level -- a materially different picture from the previous revision's confirmed METEOR loss, and we report the correction plainly rather than let the earlier, less favorable number stand uncorrected. It significantly outperforms no-retrieval on all four metrics ($p<0.001$); this comparison excludes the 1 query (0.5\%) on which the pipeline abstained -- down sharply from 10.5\% in an earlier measurement round, since the exact-match coverage fix (Section~\ref{subsec:unambiguous}) and the resulting abstention recalibration (Section~\ref{subsec:abstention}) together made the confidence gate fire correctly far less often on queries that are, in fact, answerable. An abstention is excluded rather than scored as a near-zero-BLEU miss, since that would conflate ``declined to guess'' with ``answered wrongly'' -- two outcomes this paper's abstention mechanism is specifically designed to keep distinct. We return to what this means for RQ1 in Section~\ref{sec:discussion}: on these metrics the pipeline does not clearly \emph{beat} any single-method baseline either -- parity is not an advantage -- and a companion diagnostic (Section~\ref{subsec:adaptive-isolated}) confirms the parity with BM25-only is not evidence adaptive routing itself is a sound design choice: adaptive routing's own fusion-method choice is directly shown to be significantly negative in isolation once a separate exact-match mechanism's ranking ceiling is removed as a deconfounding step. What the pipeline adds is capabilities (abstention, graph augmentation, bilingual support) the single-method baselines do not have at all, not a generation-quality win.

\subsection{Reranker Ablation}
\label{subsec:reranker-ablation}

\begin{table}[t]
\caption{Cross-encoder reranker on (fine-tuned) vs.\ off, otherwise identical pipeline, matched vs.\ full hybrid and BM25-only ($n=199$). Both rows re-measured 2026-08-01 under the current retriever (both the exact-match coverage fix, Section~\ref{subsec:unambiguous}, and the BM25 $k_1$/$b$ retuning postdate the previous measurement of each row -- see discussion below and Section~\ref{subsec:novel-vs-baselines}).}\label{tab:reranker-ablation} \centering
\begin{tabular}{|l|c|c|c|}
\hline
Reranker & Comparison & Mean diff & $p$ \\
\hline
On (fine-tuned) & vs.\ BM25-only, METEOR & $-0.023$ & \textbf{0.035} \\
Off & vs.\ Full Hybrid, ROUGE-L & $-0.017$ & 0.205 \\
Off & vs.\ BM25-only, METEOR & $-0.025$ & \textbf{0.041} (0.306) \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[t]
\caption{Banglish retrieval ablation and adaptive pipeline, re-measured 2026-08-01 under the deployed embedding model (\texttt{finetuned\_minilm\_hard\_negatives\_structured\_banglish\_expanded}) and the current retriever ($n=100$). The previous measurement (2026-07-28) predated this embedding model's own training run (2026-07-31) as well as the exact-match and BM25 fixes, and was found stale on re-verification -- see discussion below.}\label{tab:banglish-ablation} \centering
\begin{tabular}{|l|c|c|c|c|}
\hline
Variant & BLEU & ROUGE-L & BERTSc. & METEOR \\
\hline
