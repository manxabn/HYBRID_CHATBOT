...
Table~\ref{tab:reranker-ablation} reports the fair, non-confounded comparison. Fine-tuning does not flip the reranker from negative to positive: with it enabled, the pipeline loses significantly to full hybrid on BLEU, BERTScore, and METEOR ($p<0.001$, $p=0.007$, $p=0.005$; ROUGE-L is directionally worse but not significant, $p=0.078$), and its earlier significant edge over BM25-only (Table~\ref{tab:novel-sig}) disappears entirely -- with the reranker on, adaptive is merely tied with BM25-only on all four metrics rather than ahead of it on two. Every other result in this paper uses the reranker-off configuration, now with stronger evidence behind that choice than before: the negative result is not an artifact of an undertrained reranker, and disabling it is what unlocks the ROUGE-L/BERTScore advantage over BM25-only reported in Section~\ref{subsec:novel-vs-baselines}.

As a further, literature-motivated check, we tested whether the reranker's regression is a candidate-set-size effect: \citet{magomere2025} find that reranking a wide candidate pool can inject noise that disproportionately harms an already-strong first-stage ranking, and recommend reranking a smaller pool as a direct mitigation. We re-ran the fine-tuned reranker restricted to a pool of 5 candidates (equal to \texttt{final\_k}, so reranking can only reorder the already-selected top-5 rather than pull in noisier candidates from further down the ranking). This did not rescue the reranker: it remained significantly behind full hybrid on all four metrics ($p\leq0.019$), if anything a slightly worse significance profile than the wider pool-of-10 test above. We report this as evidence against candidate-set noise being the (sole) explanation for this corpus -- the reranker's joint relevance judgments disagree with the fine-tuned embeddings' ranking even when given only the five candidates the embeddings already selected as best.
