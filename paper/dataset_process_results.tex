...
\textbf{External baseline comparison.} Against ColBERTv2 \citep{santhanam2022colbertv2}, a 2022-era published late-interaction retriever, the deployed system holds a decisive advantage driven by its exact-match mechanism. Checking whether this advantage is an artifact of comparing against an aging checkpoint rather than a real property of the system, the identical methodology was repeated against \texttt{GTE-ModernColBERT-v1} \citep{chaffin2025gtemoderncolbert}, a 2025 late-interaction model reported by its authors as the first to beat ColBERT-small on the BEIR benchmark. Recall/MRR show no significant difference (both near ceiling, McNemar $p\geq0.25$ throughout), but nDCG@5/@10 -- sensitive to \emph{where} in the ranking the correct answer sits, not merely whether it appears -- are now significantly ahead for GTE-ModernColBERT under the bootstrap test. This is reported plainly as a real, current result: a specific, more recent late-interaction model can out-rank the deployed system's fine-tuned embeddings on open-ended queries (the exact-match mechanism gives the deployed system no help there), even though the deployed system retains a decisive, significant advantage over both external baselines on the entity-heavy queries that dominate this system's actual query distribution. The advantage over a 2022 checkpoint does not straightforwardly carry over to a stronger 2025 checkpoint from the same model family -- an external fact about the state of retrieval research, not a bug in this system to fix.
...
\textbf{A third faithfulness mechanism, calibrated and found unusable.} Conformal claim-level backoff \citep{mohri2024conformal} was implemented in earlier work but never calibrated. Calibrating it for the first time against a real, structurally-labeled 442-claim set (137 positive, 305 negative, both classes determined by construction rather than judgment) with a proper 70/30 held-out split finds it degenerates to a threshold of $1.0000$ -- the mathematical maximum an NLI entailment score can take -- retaining \textbf{0 of 133} held-out claims. This is not a data-quantity problem fixable by recalibration: the root cause is that $88\%$ of the calibration set's negative-labeled (incorrect) claims are near-verbatim reproductions of their own irrelevant retrieved context, so NLI entailment scores them as faithful despite the underlying answer being wrong -- the signal measures faithfulness-to-given-context, not correctness-to-the-true-query, and these diverge exactly in the unanswerable-query regime this mechanism most needs to catch. Reported as a diagnosed negative result rather than left in undecided limbo; the mechanism is not deployed.
...
\subsection{Cross-Lingual and Bilingual Performance}
\label{subsec:results-crosslingual}

The retrieval ablation (Section~\ref{subsec:results-retrieval}) shows comparable results across English and Banglish using the identical architecture, but the two components built specifically for the code-mixed setting -- cross-lingual query translation and BM25-level Banglish spelling-variant normalization -- both show null effects on the main Banglish test set. BM25 normalization's null result is a genuine tested-and-negative finding (Section~\ref{sec:limitations}), not an unexamined gap. Query translation's null result was traced to a structural blind spot in the main test set itself: most Banglish test queries ask about content that also exists in Banglish source material, so translating to English before retrieval has little left to add. A dedicated stress test targeting the scenario translation is actually meant for -- Banglish queries about content that exists only in EnglishQA, with no Banglish counterpart -- shows translation genuinely helping: top-5 retrieval accuracy improves from $61.5\%$ to $84.6\%$ ($n=13$ real, database-verified facts), confirming the null result on the main test set reflected that test set's own coverage, not evidence the technique is unhelpful. This revision's other retriever improvements (embedding fine-tuning, BM25 retuning) narrowed the margin considerably on their own -- the plain-retrieval baseline on this same stress test rose from $33.3\%$ to $61.5\%$ -- and an expanded, LLM-generated 27-query version of the same stress test no longer reaches statistical significance on the current retriever ($59.3\%$ vs.\ $63.0\%$, not significant on discordant pairs), so this finding is held more cautiously than a single positive measurement would suggest, not treated as fully settled.

\subsection{Two Further Honest Negative Results}
\label{subsec:results-further-negatives}

Two additional attempts on the open-ended abstention gate (Section~\ref{subsec:results-abstention}), tried alongside the successful gradient-boosted-tree improvement and reported regardless of outcome: replacing the binary lexical near-match signal with its continuous (non-thresholded) form produced no real effect ($0.642$ vs.\ the deployed classifier's $0.644$, not significantly different); and a fifth signal, normalized entropy of the retrieval score distribution, was tested twice -- a first, plain version showed no effect, diagnosed to a genuine cause (this corpus's fused scores cluster too tightly for raw softmax entropy to discriminate), and a parameter-free fix targeting that cause (z-scoring before the softmax) moved closer to significance but did not clear this report's own both-tests-agree bar. Neither addition was deployed; both are reported as real, diagnosed attempts rather than omitted for not working out.
...
\textbf{Table-coverage auditing has an inherent ceiling.} A standing audit verifies every source table has at least one exercising query in the test set (motivated by a real, since-closed coverage gap in FacultyAvailability); it can only ever prove a table has \emph{some} exercising query, never that its routing logic is correct in every case.

\textbf{An LLM-based entity-normalization fallback} (fuzzy/LLM correction of malformed course codes and misspelled names when exact-match finds nothing) is enabled by default and validated on a small ($n=8$) hand-constructed malformed-query set (8/8 with normalization vs.\ 1/8 without) -- a clean, reproduced signal, but a small one, disclosed as such rather than presented as broadly representative.
