...
Two further mechanisms, both enabled by default: an LLM-based entity-normalization fallback (fuzzy correction of malformed course codes and misspelled faculty names when exact-match finds nothing but an entity-shaped pattern is present), validated on a hand-constructed malformed-query set ($8/8$ resolved correctly with normalization enabled vs.\ $1/8$ without); and a standing table-coverage audit verifying every source table has at least one exercising query in the test set, introduced after a real, since-closed coverage gap was found in \texttt{FacultyAvailability}.
