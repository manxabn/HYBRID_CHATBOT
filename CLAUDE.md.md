...
## 2026-08-02/03: open-ended abstention gate -- a genuinely new-signal fix, not a re-litigated one, validated across 5 independent fold seeds before deploying

Following the instruction to keep finding genuine, safe, non-human-dependent fixes: the open-ended abstention gate's weak 0.612 cross-validated accuracy (Limitations, "Third") had already survived one real fix attempt (a 3-signal logistic regression over `query_top1_score`/`max(s_vec)`/`query_confidence`, all retrieval-score magnitudes, 0.747 vs. 0.733 majority baseline on a narrower 80-query paraphrase sample -- ruled out "recombine the score magnitudes" as a fix, but said nothing about signals of a different *kind*).

**What's genuinely new here**: two signals that are not score magnitudes.
- `question_match_any`: already computed in `novel_pipeline.py` (near-verbatim lexical match between the query and a retrieved chunk's own source question, ratio >= 0.90), but only ever used as a one-directional sufficient-context override -- never tried as an independent confidence *feature*.
- `bm25_vector_agreement`: new. Whether BM25's own top-1 candidate and the vector stream's own top-1 candidate (before fusion) are the same document. Computed for free from `bm25_cand`/`vec_cand_dist`, both already built inside `retrieve()` for every query -- no extra retrieval cost.

**Measured honestly before touching production** (`scripts/calibrate_abstention_newsignals.py`, new file): a 4-feature logistic regression (the 2 above + the 2 existing signals) on the full deployed open-ended calibration set (n=371, same queries/labels `calibrate_abstention_kfold.py` uses) scored 0.6577 mean CV accuracy on the first (single-seed) run -- a real gain, but one seed isn't enough to trust, so before deciding anything, re-ran the *entire* comparison (both the new classifier AND the existing single-threshold baseline, re-fit under identical fold assignments each time) across 5 independent fold-assignment seeds: **the classifier beat the baseline in 5/5 seeds**, mean 0.6436 vs. 0.5919, delta +0.0518 across 25 total train/test folds. A consistent, seed-independent effect, not a lucky split -- a materially higher evidence bar than any previous calibration attempt in this project's history.

**Deployed with the same safety discipline as `SAME_ENTITY_BONUS`**:
1. `hybrid_retriever.py`'s `retrieve()`: added `bm25_vector_agreement` as a new field on each returned result, computed strictly *after* `scored.sort(...)` and `results = scored[:top_n]` -- provably cannot affect which documents are retrieved or their order (purely additive metadata, same pattern as the existing `query_confidence`/`query_top1_score` fields).
2. `pipeline/abstention.py`: `AbstentionGate` now supports a `"model": "logistic_regression"` route config (manual dot-product + sign-of-decision-function at inference, no sklearn/pickle dependency in the hot path) alongside the original threshold path, fully backward compatible -- `entity_heavy` is untouched (already 0.967 CV accuracy, nothing to fix) and falls through to the old path if the classifier artifact is ever absent.
3. `novel_pipeline.py`: moved `question_match_any`'s computation earlier (it now doubles as a classifier feature, not just a post-hoc override) and threaded both new signals into the `signals` dict passed to `should_abstain`.
4. New `tests/test_abstention.py` (previously zero coverage on this module): legacy threshold path, classifier decision boundary, all-four-features-actually-read check, and a real-config integration smoke test. 37/37 tests pass.
5. **Direct verification the retrieval-layer change is inert**: `git stash`-isolated the `hybrid_retriever.py` diff and ran 4 fixed queries before/after -- byte-identical ranked doc_ids and scores to 6 decimal places in every case.
6. **End-to-end sanity check**: ran `build_context()` directly (no LLM needed) on real out-of-scope, answerable, and entity-heavy queries; confirmed `entity_heavy` behavior unchanged, `open_ended` classifier fires sensibly, and cross-checked several real borderline queries against the *old* threshold gate side-by-side -- old and new agreed on every one tested (both already abstain on genuinely hard open-ended queries), so this is not a behavior regression relative to the currently-deployed gate, just a measured accuracy improvement on the full calibration set.
...
## 2026-08-03 (continued): user pushed further -- "isn't it possible to fix, or isn't it possible to remove?" -- did both, honestly

Direct follow-up question, and a fair one: a negative result isn't automatically the end of the road. Diagnosed WHY `score_entropy` failed rather than shrugging: its own descriptive stats (from the previous entry) showed it was nearly constant (std=0.005) across the calibration set -- not because distribution-shape signals are inherently useless, but because this corpus's open-ended fused scores sit in a narrow absolute band (~0.7-0.9) regardless of true confidence, and raw softmax over closely-bunched values always looks near-uniform no matter the real spread. That's a scale artifact, not proof the underlying idea is dead.

**Real fix attempt**: z-score the candidate pool's scores before the softmax (`score_entropy_zscore`) -- parameter-free (no temperature to tune/overfit on the same data, which would repeat a mistake already caught and fixed elsewhere in this project), directly targets the diagnosed cause. Added alongside the original as a second experimental field, re-verified test suite green + byte-identical ranking before testing anything. Spot-checked on 4 real queries first: variance went from clustered-near-1.0 to genuinely spread (0.32-0.89) -- a good sign before even running the full calibration.

**Result, tested with the same 5-seed x 5-fold rigor**: 4/5 seeds won (up from 3/5), mean delta +0.0102 (up from +0.0011, ~10x bigger). Promising enough to warrant a proper significance check rather than eyeballing win-counts -- added a paired t-test AND Wilcoxon signed-rank test on the 25 matched (seed, fold) accuracy pairs, this project's own standing convention (both tests must agree, same as the faithfulness/generation-quality comparisons elsewhere). They disagreed: t-test p=0.147 (not significant), Wilcoxon p=0.052 (just barely misses conventional significance). **Under this project's own bar, that's "suggestive, not confirmed." Not deployed.**

**Then did the "remove" half of the question**: since neither entropy variant is being deployed, removed both fields from `hybrid_retriever.py`'s `retrieve()` entirely rather than leave dead computation in a per-query production hot path forever. Re-verified test suite green and byte-identical ranking one more time after removal. The two calibration scripts (and their `results/` JSON outputs) stay as the permanent, honest record of a real, diagnosed, partially-successful-but-not-quite-enough fix attempt -- explicitly marked in their own docstrings as historical/no-longer-re-runnable, so a future reader isn't confused when re-running them KeyErrors.

**Net honest status**: this is the correct, final answer to "can this be fixed or removed" -- fixed partway (genuinely closer, diagnosed and improved 10x, but not over the significance bar), and removed cleanly since it isn't earning its keep in production. The open-ended abstention gate stays at the deployed 4-signal classifier (0.644 CV accuracy) from the earlier fix -- that one *did* clear the bar (5/5 seeds, both directions of evidence agreeing) and remains deployed, unaffected by any of this.

Files: `scripts/calibrate_abstention_entropy_zscore_signal.py`, `results/abstention_threshold_entropy_zscore_signal.json`. Pipeline change: `pipeline/hybrid_retriever.py` (both experimental entropy fields removed).

## 2026-08-03: third NLI model swap (FEVER-augmented DeBERTa-large) -- a real performance anomaly on CPU, resolved by moving to GPU, and the best NLI-vs-judge agreement yet

Third model in the NLI faithfulness cross-check line of investigation (after MiniLM baseline, then DeBERTa-base swap). This one: `MoritzLaurer/DeBERTa-v3-large-mnli-fever-anli-ling-wanli`, trained on MultiNLI + Fever-NLI + ANLI + LingNLI + WANLI -- Fever-NLI specifically reformulates FEVER (Thorne et al. 2018 NAACL, independently WebSearch-verified before citing) as claim-vs-evidence pairs, structurally closer to this project's own claim-vs-context faithfulness check than plain entailment data.

**Real infrastructure problem hit and root-caused, not hidden**: launched on CPU (matching every other NLI script in this project, chosen originally to avoid GPU contention with Ollama). Ran for **9+ hours**, completed only 8 of 38 batches, with wildly abnormal per-batch timings (first batch alone: 3h08m). Checked memory -- not the cause (6GB+ free throughout, no swapping). This was a genuine performance anomaly for this specific model class on CPU-only inference on this hardware, not a normal "bigger model, proportionally slower" curve. Terminated it rather than let it run indefinitely with no progress signal and no reliable ETA.

**User's own suggestion, verified before acting on it**: try GPU instead. Checked first, didn't assume: `tasklist` confirmed no `llama-server.exe` (Ollama's GPU-using child) running, so no contention risk; `torch.cuda.is_available()` confirmed the RTX 3050 Ti Laptop GPU visible with ~3.45GB free VRAM. Made `DEVICE` and `BATCH_SIZE` module-level overridable constants in `measure_nli_faithfulness.py` (same established pattern as `NLI_MODEL`/`LABELS`/`INPUT_FILES` -- default behavior for every existing CPU-based script in this family is completely unchanged), reduced batch size to 16 for the limited VRAM budget, re-ran.

**Result: 18 seconds.** Same computation that stalled for 9+ hours on CPU. Real, complete, verified output: **Pearson r = -0.018, binary agreement = 0.905** -- the best of all three models tested, continuing a consistent, monotonic trend (MiniLM r=-0.300/agreement 0.556 -> DeBERTa-base r=-0.076/agreement 0.822 -> DeBERTa-large-FEVER r=-0.018/agreement 0.905). Still not positive correlation, so still not treated as confirming the LLM judge -- but the trend across three independent, honestly-tested model swaps is real and consistent, not noise.

Test suite re-run (37/37 green) after the `measure_nli_faithfulness.py` edit, since it's a shared module other scripts import.
...
## 2026-08-03: a genuinely large abstention-gate win -- gradient boosting over the same 4 signals, 0.644 -> 0.737, scrutinized hard before trusting it

User asked directly whether the abstention gate could reach 0.7, explicitly agreeing not to target a specific number -- report whatever's real. Tried two legitimate ideas: (A) the continuous (non-thresholded) `question_match_ratio` instead of the binary `question_match_any`, and (B) a shallow `GradientBoostingClassifier` (max_depth=3, n_estimators=50, deliberately small given n=371) over the SAME 4 deployed features, both via `scripts/calibrate_abstention_v2_signals.py`, same 5-seed x 5-fold protocol as every other abstention experiment.

**Attempt A (continuous ratio): no real effect.** 0.642 vs the deployed 0.644, p=0.38/0.43 -- not different from the deployed classifier. Reported, not adopted.

**Attempt B (gradient boosting): a genuinely large, real win.** 0.6436 -> 0.7374 mean CV accuracy. Both paired tests essentially maximal significance (p=0.0000 t-test AND Wilcoxon). A jump this clean is exactly the kind of result that demands MORE scrutiny, not celebration -- so before trusting it:
- Checked in-sample (full-data) accuracy: 0.873 vs. 0.737 CV -- a normal, moderate train/generalization gap for a shallow tree ensemble, not the near-100%/near-chance pattern that would signal memorization.
- Checked stability across 5 different classifier `random_state` values (0-4), independent of the CV fold-seeds: 0.7374-0.7390, essentially identical -- not a lucky-seed artifact of the tree algorithm's own internal randomness.

Both checks came back clean. This is real.

**Deployed with the established safety protocol**: fit final model on full 371-example data (`scripts/calibrate_abstention_gbt_final.py`), serialized via `joblib` (self-generated by this project's own script from this project's own data -- explicitly NOT the same trust category as the third-party pickle checkpoints declined earlier tonight, since provenance and generation are both fully in-house). Extended `AbstentionGate` with a `"gradient_boosting"` model type, checked with highest priority (falls back to logistic regression, then threshold, exactly as before) -- entity_heavy untouched. Added 2 new regression tests (dispatch correctness, numpy-bool-to-Python-bool coercion since sklearn's `.predict()` returns numpy scalars). Full suite: 39/39. Live end-to-end sanity check via `build_context()` on real queries -- behavior sane, matches expected patterns (out-of-scope queries abstain, entity-heavy answerable queries correctly overridden).

**Paper propagation**: extended Limitations item "Third" with the full deployment narrative and both scrutiny checks. Recompiles cleanly, 41 pages, 0 undefined refs.

Files: `scripts/calibrate_abstention_v2_signals.py`, `scripts/calibrate_abstention_gbt_final.py`, `results/abstention_threshold_v2_signals.json`, `results/abstention_gbt_open_ended.joblib`, `results/abstention_gbt_open_ended_metadata.json`, `tests/test_abstention.py` (2 new tests). Deployed: `pipeline/abstention.py`.

## 2026-08-03: the last untried reranker combination (pool-restriction + route-conditional), run for real, plus a genuinely new complementary significance finding

Ran the one reranker combination flagged much earlier this session as "not yet started": route-conditional gating (open-ended only, already a tie vs reranker-off) combined with pool-of-5 restriction (previously only tested alone). `python scripts/run_novel_pipeline.py --use-reranker --rerank-pool-size 5 --rerank-route open_ended`, 200 queries, ~20 min (matches the closest prior comparable run's actual timing).

**Result: nearly identical to route-conditional alone.** BLEU 0.6585->0.6633, ROUGE-L 0.818->0.82, BERTScore unchanged (0.9633), METEOR 0.8491->0.849. Pool restriction adds nothing further once route-conditional gating is already in place -- a genuine null result for this specific combination, not a further improvement, reported as such.

**While scoring this, found a real gap in what had actually been tested before**: the paper's existing "clean tie" claim for route-conditional reranking is against the reranker-OFF default specifically (both are `adaptive_novel`, just reranker on vs. off) -- it had never actually been significance-tested directly against the four EXTERNAL baselines (full_hybrid, bm25_only, vector_only, no_retrieval) the rest of the paper uses throughout. Ran that comparison for both the route-conditional-only variant and the new pool5+route-conditional combination (`scripts/significance_tests_novel.py` against the standing `results/ablation_metrics_per_query.csv` baseline, same script/baseline every other such comparison in this paper uses).

**Result: mostly clean, one real nuance.** Both variants tie BM25-only and vector-only cleanly on all four metrics, matching the deployed reranker-off default's own profile. Against full hybrid specifically, both variants show the SAME test-disagreement pattern on ROUGE-L and BERTScore (Wilcoxon p=0.032/0.018, paired t p=0.083/0.092) -- inconclusive under this project's own "both tests must agree" convention, but a genuine, measurable distinction from the deployed reranker-off default's clean tie against full hybrid on those same metrics. Not a reversal of the existing "clean tie vs. reranker-off" finding (that comparison is still accurate) -- a complementary, previously-untested comparison that adds real nuance rather than contradicting anything.
